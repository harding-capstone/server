...
import com.shepherdjerred.capstone.server.network.message.EventMessage;
...
public class ReceivedMessageEventHandler implements EventHandler<ReceivedMessageEvent> {
  ...
  public void handle(ReceivedMessageEvent event) {
    ...
    if (message instanceof PlayerInitializationMessage) {
      handlePlayerInfoMessage((PlayerInitializationMessage) message, connection);
    } else if (message instanceof EventMessage) {
      handleEventMessage((EventMessage) message, player);
    }
  }

  private void handlePlayerInfoMessage(PlayerInitializationMessage message, Connection sender) {
